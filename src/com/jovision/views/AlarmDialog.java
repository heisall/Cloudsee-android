	...
	android.view.View.OnClickListener myOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.dialog_view:
			    ...
			    String strTempNameString = contextString.substring(contextString.lastIndexOf(".")+1,contextString.indexOf("@")); 
...
//					playIntent.putExtra("DeviceIndex", getDeivceIndex(ystNum));
	...
	private int getDeivceIndex(String strYstNum){
		...
		for (int j = 0; j < CacheUtil.getDevList().size(); j++) {
			...
			if (strYstNum.equalsIgnoreCase(devs.getFullNo())) {
				bfind = true;
				MyLog.e("New Alarm Dialog", "find dev num "+strYstNum+", index:"+j);
				return j;
			}
		}
		return -1;
